...
### Package: `@insforge/sdk@1.4.2`

Sub-exports used:
- `@insforge/sdk/ssr` — SSR auth helpers: `createBrowserClient`, `createServerClient`, `createAuthActions`, `createRefreshAuthRouter`, `updateSession`
- `@insforge/shared-schemas` — Auth type schemas (e.g. `OAuthProvidersSchema`)
...
// Reads NEXT_PUBLIC_INSFORGE_URL and NEXT_PUBLIC_INSFORGE_ANON_KEY from env automatically
...
```typescript
// lib/cookie-adapter.ts — shared adapter wrapping Next.js cookies()
// Required because Next.js cookie store has overloaded set/delete signatures
// that don't match InsForge's CookieStore interface directly.
...
export async function createCookieAdapter(): Promise<CookieStore> {
  ...
  return {
    ...
    set: (...args: unknown[]) => { (store.set as any)(...args); },
    delete: (...args: unknown[]) => { (store.delete as any)(...args); },
  };
}
```
...
// Step 1: Initiate OAuth — returns provider URL + codeVerifier
export async function signInWithOAuth(provider: "google" | "github") {
  const cookieAdapter = await createCookieAdapter();
  const auth = createAuthActions({ cookies: cookieAdapter });

  const { data, error } = await auth.signInWithOAuth(provider, {
    redirectTo: `${process.env.NEXT_PUBLIC_INSFORGE_URL}/callback`,
    skipBrowserRedirect: true, // required for server action flow
  });

  // Store codeVerifier in httpOnly cookie (path: /callback, maxAge: 10min)
  // Then redirect browser to data.url
}

// Step 2: Exchange code in Route Handler (not a page — server components can't write cookies)
// app/api/auth/callback/route.ts — reads insforge_code from URL params
import { createAuthActions } from "@insforge/sdk/ssr";
...
export async function GET(request: NextRequest) {
  ...
  const auth = createAuthActions({
    requestCookies: { get: (name) => request.cookies.get(name)?.value ?? null },
    responseCookies: response.cookies, // wraps set/delete for Set-Cookie headers
  });
...
```typescript
// proxy.ts — route protection + token refresh (Next.js 16 proxy convention)
import { updateSession } from "@insforge/sdk/ssr";

const { accessToken } = await updateSession({ requestCookies, responseCookies });
...
**Cookie names (InsForge defaults):**
- `insforge_access_token` — access token
- `insforge_refresh_token` — refresh token
- `insforge_pkce_verifier` — custom PKCE verifier (10min maxAge, path: /callback)

**Rules:**

- Always use `skipBrowserRedirect: true` with `signInWithOAuth` in server actions
- `redirectTo` must point to the app's callback URL (e.g. `NEXT_PUBLIC_APP_URL/api/auth/callback`) — never the InsForge backend URL
- PKCE callback must be a **Route Handler** (`app/api/auth/callback/route.ts`), not a page component — `exchangeOAuthCode` writes session cookies which requires `Set-Cookie` headers on a Response object
- For Route Handlers use `requestCookies`/`responseCookies` pattern; for Server Actions use `cookies`
- Always store codeVerifier in httpOnly cookie — never in client state. Cookie `path` must match the callback Route Handler path (e.g. `/api/auth/callback`)
- PKCE verifier cookie should have short maxAge (10min) and be deleted after exchange
