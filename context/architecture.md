...
│   │   │   └── LoginForm.tsx              → Client component with OAuth buttons
│   ├── dashboard/
...
│       ├── auth/
│       │   ├── callback/
│       │   │   └── route.ts              → OAuth callback handler (Route Handler for cookie writes)
│       │   └── refresh/
...
│   ├── cookie-adapter.ts                  → Next.js cookies() adapter for InsForge CookieStore
...
- Public routes: /, /login
- Proxy in proxy.ts checks session on every protected route (Next.js 16 replaces middleware.ts with proxy.ts)
...
Cookie adapter (`lib/cookie-adapter.ts`) wraps Next.js `cookies()` to match the InsForge `CookieStore` interface, handling overloaded `set`/`delete` signatures.
